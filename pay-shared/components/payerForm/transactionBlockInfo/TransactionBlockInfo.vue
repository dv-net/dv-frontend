<script setup lang="ts">
	...
	const {
		...
		currentTransaction: IWalletTransactionResponse;
	...
	const getLabelBlockchain = (blockchain: "token" | "chain"): string | null => {
		if (blockchain === "token") {
			const find = addresses.find(
				(item) => getCurrentCoin(currentTransaction.currency_code) === getCurrentCoin(item.currency.id)
			);
			return find?.currency?.currency_label ? `(${find.currency.currency_label})` : null;
		} else if (blockchain === "chain") {
			const find = addresses.find((item) => currentTransaction.currency_code === item.currency.id);
	...
	<div class="transaction">
		...
		<div class="row">
			...
			<span class="row__value">
				...
				{{ getCurrentCoin(currentTransaction.currency_code) }} ({{ formattedFiatAmount }})
			</span>
		</div>
		<div class="row">
