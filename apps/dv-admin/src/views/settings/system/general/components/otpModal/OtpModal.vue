<script setup lang="ts">
	...
	const { setting } = defineProps<{ setting: ISettingsResponse }>();
	...
	const handleSendForm = async () => {
		try {
			...
			await postApiUserSettings(setting, otp.value.join(""));
			clearFormData();
			notify(t("The settings for sending funds have been changed"), "success");
		} catch (error: any) {
			console.error(error);
		} finally {
...
<template>
	<ui-modal v-model="isShow" @close="handleCloseModal" width="500">
